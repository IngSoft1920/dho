...
public class HabitacionDAO { 
	...
	//de momento un cliente solo tiene una estancia a su nombre -> luego ampliar para que con un id_cliente se puedan guardar varias estancias
	public static int diasReserva(int cliente_id) { 
		...
		ResultSet rs=null; 
		 try {
		stmt=conexion.getConexion().createStatement(); 
		rs=stmt.executeQuery("SELECT DATEDIFF(fecha_fin, fecha_inicio) as n FROM Estancia WHERE cliente_id=" +cliente_id);
		 if(rs.next()) {
			  res=rs.getInt("n");
		 }
		 }catch (SQLException ex){ 
	...
	///Dado habitacion_id devuelve el HabitacionBean
		...
		public static ArrayList<HabitacionBean> getHabitacionByHotel (int hotel_id){
			...
			try { 
				...
				while(rs.next()){
					res.add(new HabitacionBean (rs.getInt("habitacion_id"), 
							rs.getInt("hotel_id"), 
